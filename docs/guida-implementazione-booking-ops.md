# Guida implementazione — Booking ops AVES SDK 1.5.0

Guida pratica per le operazioni di booking (aves-sdk@1.5.0), derivate dallo spike API AVES XML 1.8.0.
...
- `selectedServiceList.selectedServiceDetail` è un **array** (shape Mod ≠ Create).
...
> Shape lista servizi: **Create** = array di wrapper `{ selectedServiceDetail: one }`; **Mod** = `{ selectedServiceDetail: one[] }`.
...
const paid = await client.insertFilePaymentList({
  ...
  operationType: "AbsoluteAmountsInsertion",
  filePaymentList: {
    filePaymentDetail: leadPayments.map((p) => ({
      paymentDate: p.date,
      paymentNote: p.note,
      amount: p.amount,
      paymentType: p.type,
