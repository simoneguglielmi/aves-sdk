...
describe("BookingFileSchema", () => {
	it("should validate minimal booking file (required fields only)", () => {
		...
		expect(result.selectedServiceList.selectedServiceDetail).toHaveLength(1);
		expect(result.passengerList.passengerDetail).toHaveLength(2);
	});
	...
	it("should validate single selectedServiceDetail (non-array)", () => {
		...
		expect(result).toBeDefined();
		expect(result.selectedServiceList.selectedServiceDetail).toHaveLength(1);
		expect(result.passengerList.passengerDetail).toHaveLength(1);
	});
...
describe("BookingFileApiSchema", () => {
	it("should transform camelCase input to PascalCase with @ for attributes", () => {
		...
		expect(result.SelectedServiceList.SelectedServiceDetail).toHaveLength(1);
		expect(result.SelectedServiceList.SelectedServiceDetail[0]).toHaveProperty(
			"@SCode",
			"S1",
		);
		expect(result.PassengerList.PassengerDetail[0]).toHaveProperty(
			"@Rph",
			"001",
		);
		expect(result.PassengerList.PassengerDetail[0]).toHaveProperty(
