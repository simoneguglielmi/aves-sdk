...
import type { InferInput } from '../effect/infer.js';
...
/**
 * Pin Context to `never` for schemas Effect infers with `unknown` Context
 * (dynamic `Struct` keys / `partial`). These schemas never require services.
 */
...
export function createApiSchema<A extends object, I extends object>(
  ...
  shape: WireShapeFor<A> | WireShape,
...
type MappedFacades<
	...
	A extends Readonly<Record<string, string>>,
> = {
	[K in keyof A as A[K] extends keyof TFields ? K : never]: A[K] extends keyof TFields
...
function optionalizeField(schema: StructField): StructField {
	...
	return Schema.optional(
		Schema.asSchema(schema as Schema.Schema.AnyNoContext),
	);
}
...
function omitUndefinedUnion(ast: SchemaAST.AST): SchemaAST.AST {
	...
	const parts = ast.types.filter((t: SchemaAST.AST) => t._tag !== "UndefinedKeyword");
...
function unwrapOptional(schema: StructField): Schema.Schema.AnyNoContext {
	if (!Schema.isPropertySignature(schema)) {
		if (Schema.isSchema(schema))
			return syncSchema(Schema.asSchema(schema));
...
export function createApiValidationSchema<TFields extends StructFields>(
  inputSchema: Schema.Struct<TFields>,
  shape: WireShapeFor<InferInput<typeof inputSchema>> = {},
  overrides?: StructFields,
): Schema.Schema.AnyNoContext {
